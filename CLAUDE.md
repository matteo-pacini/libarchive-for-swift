...
This is a Swift Package Manager distribution of [libarchive](https://libarchive.org/) for Apple platforms. It packages libarchive (v3.7.9) and its dependencies as a static XCFramework with all symbols bundled together.

**Bundled dependencies:** zlib (1.3.1), bzip2 (1.0.8), xz/liblzma (5.8.1), zstd (1.5.7), lz4 (1.10.0)
...
### Patches
Custom patches in `patches/` handle Apple platform compatibility:
...
- `0003-xz-apple-support.patch`: xz compatibility for Apple platforms

### Platform Limitation
External process spawning via `archive_write_add_filter_program()` only works on macOS. Other Apple platforms gracefully fail these operations due to sandbox restrictions.
